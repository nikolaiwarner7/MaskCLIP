...
## CHANGE FOR EACH RUN:
RUN_NAME = 'RGBSI_COCO_Test_2_19'
SAMPLES_PER_GPU = 16
INIT_LR = 5e-3

#
EVAL_INTERVAL = [2000] # Change to 1000, set to 1 for debugging eval hook
...
def parse_args():
    ...
    parser.add_argument('--config', default='configs/maskclip_plus/zero_shot/maskclip_plus_r50_deeplabv2_r101-d8_512x512_80k_coco-stuff164k.py', help='test config file path')
...
def main():
    ...
    # Should also be unchanged for RGBS, I training
    ...
    cfg.data.train.img_dir = 'images/Inst_Seg_RGBSI_Images/train'
    cfg.data.train.ann_dir = 'annotations/Inst_Seg_RGBSI_Annotations/train'
    cfg.data.train.split = 'image_sets/RGBS+I_train.txt'
    # This config data below only used to set datatype to npy, so should be recyclable 
    cfg.data.train.type = 'COCOStuffDataset'
    ...
    cfg.data.val.img_dir = 'images/Inst_Seg_RGBSI_Images/val'
    cfg.data.val.ann_dir = 'annotations/Inst_Seg_RGBSI_Annotations/val'
    cfg.data.val.split = 'image_sets/RGBS+I_val.txt'
    # This config data below only used to set datatype to npy, so should be recyclable 
    cfg.data.val.type = 'COCOStuffDataset'
    ...
    # Update model to accept 5CH inputs for RGBSI
    model.backbone.stem[0] = nn.Conv2d(5, 32, kernel_size=(3,3), stride=(2,2), padding=(1,1))
